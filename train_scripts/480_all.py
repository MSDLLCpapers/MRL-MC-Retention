...
def main(args):
    ...
    for i in range(epochs):
        ...
        scheduler.step()
    best_model_stat = copy.deepcopy(model.state_dict())
    save_log_file(best_model_stat, log_file, 'output/480_all', 1)
    pred_summary = return_prediction(model, best_model_stat, device, "./output/480_all", loss_fn, loss_MAE, 0, big_test_dataloader, 'test')
    pred_summary = return_prediction(model, best_model_stat, device, "./output/480_all", loss_fn, loss_MAE, 0, test_dataloader, 'val')
...
def save_log_file(best_model_stat, log_file, file_savepath, fold):
    ...
    torch.save(best_model_stat, os.path.join(file_savepath, f"all_data_model_weight.pth"))
