...
true_y = pd.read_csv('data/F2_labels.txt').true_y.values
...
def main(args):
    ...
    model.to(device)
    raw_dir = "./data"
    train_dataset = SMRTDatasetOneHot(name="F2_train", raw_dir=raw_dir)
    test_dataset = SMRTDatasetOneHot(name="F2_val", raw_dir=raw_dir)
    big_test_dataset = SMRTDatasetOneHot(name="F2_test", raw_dir=raw_dir)
    '''data_loader'''
    ...
    for i in range(epochs):
        ...
        if test_loss < best_loss:
            ...
            if es > early_stop:
                ...
                break
    save_log_file(best_model_stat, log_file, 'output/no_train', 1)
    pred_summary = return_prediction(model, best_model_stat, device, "./output/no_train", loss_fn, loss_MAE, 0, big_test_dataloader, 'test')
    pred_summary = return_prediction(model, best_model_stat, device, "./output/no_train", loss_fn, loss_MAE, 0, test_dataloader, 'val')
...
def save_log_file(best_model_stat, log_file, file_savepath, fold):
    ...
    result.to_csv(os.path.join(file_savepath, f"F1_log_file.csv"))

    # print min
    index = result.iloc[:,3].idxmin(axis =0)
    print("the index of min loss is shown as follows:",result.iloc[index, :])
    torch.save(best_model_stat, os.path.join(file_savepath, f"F2_model_weight.pth"))
...
def return_prediction(model, best_model_stat, device, file_savepath, loss_fn, loss_MAE, fold, dataloader, prefix):
    ...
    result.to_csv(os.path.join(file_savepath ,f"F2_{prefix}_results.csv"))
    print(y)
    print(pred)
    rt_summary = {
