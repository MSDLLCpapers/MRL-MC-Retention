...
def main(args):
    ...
    model.to(device)
    raw_dir = "./data"
    train_dataset = SMRTDatasetOneHot(name="PHEN_F2_train", raw_dir=raw_dir)
    test_dataset = SMRTDatasetOneHot(name="PHEN_F2_val", raw_dir=raw_dir)
    big_test_dataset = SMRTDatasetOneHot(name="PHEN_F2_test", raw_dir=raw_dir)
    '''data_loader'''
...
def save_log_file(best_model_stat, log_file, file_savepath, fold):
    ...
    result.to_csv(os.path.join(file_savepath, f"PHEN_F2_log_file.csv"))

    # print min
    index = result.iloc[:,3].idxmin(axis =0)
    print("the index of min loss is shown as follows:",result.iloc[index, :])
    torch.save(best_model_stat, os.path.join(file_savepath, f"PHEN_F2_model_weight.pth"))
...
def return_prediction(model, best_model_stat, device, file_savepath, loss_fn, loss_MAE, fold, dataloader, prefix):
    ...
    result.to_csv(os.path.join(file_savepath ,f"PHEN_F2_{prefix}_results.csv"))
    print(y)
    print(pred)
    rt_summary = {
