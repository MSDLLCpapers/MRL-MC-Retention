...
def get_edge_dim(exclude_feature=None):
    ...
    if exclude_feature:
        ...
        edge_dim = len(feature_ops.bond_featurizer(mol.GetBonds()[0], exclude_feature))
    # print("Coding: edge dim ", edge_dim)
...
def get_node_features(mol, exclude_feature=None):
    ...
    #     a = feature_ops.atom_featurizer(atom,exclude_feature) 
    #     print("Coding: AAA ", i, a.shape, a)

    node_features = np.array([
        feature_ops.atom_featurizer(atom,exclude_feature) for atom in mol.GetAtoms()
    ], dtype='float32')

    # # ROS: check INF in array
    # print("Coding: type node_features ", type(node_features))
    # for i in range(node_features.shape[0]):
    #     for j in range(node_features.shape[1]):
    #         if not np.isfinite(node_features[i, j]):
    #             print("Coding: inf ", i, j)

    # logger.info({"node_dim": node_features, "exclude_feature": exclude_feature})
    # print("Coding: node features ", node_features.shape, np.unique(node_features))
...
def get_edge_features(mol, exclude_feature=None):
    ...
    # print("Coding: edge features ", edge_features)
...
def smiles2graph(smiles_string,  exclude_node=None, exclude_edge=None):
    ...
    # for i in graph.keys():
    #     print("Coding: ", type(graph[i]))
    #     if isinstance(graph[i], np.ndarray):
    #         print("Coding: iii ", i, graph[i].shape)
    #     else:
    #         print("Coding: iii ", i, graph[i])
...
class SMRTDatasetOneHot(DGLDataset):
    ...
    def _load_graph(self, filename):
        ...
        # print("Coding: ", self.train_set.shape)
        # for i, r in self.train_set.iterrows():
        #     print("Coding: ", i, r["smiles"], "SEP", r["RT"])
    ...
    def __getitem__(self, idx):
        ...
        """
        # ROS added
        # print("Coding; graph data ", idx, self.graphs[idx], self.label[idx])
