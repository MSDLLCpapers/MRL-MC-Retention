...
def atom_featurizer(atom: Chem.Atom, exclude_feature) -> np.ndarray:
    ...
    #     t_as = globals()[atom_feature](atom)
    #     print("Coding: feature ", atom_feature,  len(globals()[atom_feature](atom)), t_as) 
    #     for i, t in enumerate(t_as):
    #         print("Coding: inff ", j, i, atom_feature, np.isfinite(t_as[i]))
    #         j+= 1
    return np.concatenate([
